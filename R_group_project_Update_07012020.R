...
Analytic_dataset <- read_sas("AnalyticDataInternetGambling.sas7bdat")
Demographics <- read_sas("RawDataIDemographics.sas7bdat")
PokerChip <- read_sas("RawDataIIIPokerChipConversions.sas7bdat")
DailyAggregation <-read_sas("RawDataIIUserDailyAggregation.sas7bdat")
...
Application_ID<-read_excel("Application_ID.xlsx")
...
write.csv(datamartFinal,'Final_Datamart.csv', row.names = FALSE)
write.csv(DailyAggregation,'Daily_Aggregation.csv', row.names = FALSE)
